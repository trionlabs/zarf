 ...
 * BYTE-IDENTICAL to the Rust `MerkleAirdrop` contract. Both runtimes
 * independently rebuild the same protocol-root anchor; no generated
 * fixture file or duplicated proof list is required. This script does NOT
 * re-implement hashing/XDR — it calls the audited @zarf/core implementation.
 *
 ...
 *   --self-test            ignore input; rebuild the frozen protocol root and
 *                          verify every generated proof
...
/**
 * Rebuild the protocol anchor and every proof at runtime. The Rust contract
 * suite independently computes this same root, so equality pins the external
 * byte protocol without checking in generated leaves or proof arrays.
 */
function selfTest(core) {
  const { buildTree, leafHash, verifyProof, normalizeAirdropAddress } = core;
  const rows = PROTOCOL_ROWS.map((row) => ({
    address: normalizeAirdropAddress(row.address),
    amount: row.amount,
  }));
  const tree = buildTree(rows);
  if (tree.root !== PROTOCOL_ROOT) {
    die(
      `self-test FAIL root\n` +
        `  expected ${PROTOCOL_ROOT}\n  got      ${tree.root}`,
    );
  }
  rows.forEach((row, index) => {
    const leaf = leafHash(index, row.address, row.amount);
    if (
      leaf !== tree.leaves[index] ||
      !verifyProof(tree.root, leaf, tree.proofs[index])
    ) {
      die(`self-test FAIL proof @${index}`);
    }
  });
  ...
  const manifest = resolve(HERE, "../../airdrop/Cargo.toml");
  const interop = spawnSync(
    ...
    [
      ...
      "--test",
      "airdrop",
      "typescript_proofs_are_accepted_by_contract",
      "--",
      "--ignored",
      "--exact",
      "--nocapture",
    ],
    {
      ...
      env: {
        ...process.env,
        ZARF_TS_MERKLE_BUNDLE: JSON.stringify(contractBundle),
      },
    },
  );
  if (interop.error || interop.status !== 0) {
    die(
      "TypeScript -> Soroban interop failed\n" +
        (interop.error ? `  ${interop.error.message}\n` : "") +
        interop.stdout +
        interop.stderr,
    );
  }
  ...
  logErr(
    `merkle-proof-gen self-test OK: 1 root, ${rows.length} runtime-built proofs, ` +
      `actual contract accepted TS bundle`,
  );
}
