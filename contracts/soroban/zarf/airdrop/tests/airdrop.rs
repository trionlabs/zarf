...
use soroban_sdk::{
    ...
    token, vec,
    xdr::ToXdr,
    Address, Bytes, BytesN, Env, Event, IntoVal, MuxedAddress, String as SString, Vec,
};
...
fn bytes32(env: &Env, value: &str) -> BytesN<32> {
    ...
    for (index, byte) in bytes.iter_mut().enumerate() {
        *byte = u8::from_str_radix(&hex[index * 2..index * 2 + 2], 16).expect("valid hex");
    }
    BytesN::from_array(env, &bytes)
}

fn assert_protocol_tree(env: &Env, rows: &[(Address, i128)], expected_root: &str) {
    let leaves: StdVec<BytesN<32>> = rows
        .iter()
        .enumerate()
        .map(|(index, (claimant, amount))| leaf(env, index as u32, claimant, *amount))
        .collect();
    let (root, proofs) = build_tree(env, &leaves);

    assert_eq!(hex0x(&root.to_array()), expected_root);
    for (candidate, proof) in leaves.iter().zip(proofs.iter()) {
        let mut computed = candidate.clone();
...
#[test]
fn merkle_protocol_root_is_stable() {
    // One immutable checksum is enough to pin the external byte protocol. This
    // five-leaf shape covers G/C address XDR, full-width amounts, balanced
    // pairing, and both leaf/internal odd-node promotion. Leaves and proofs are
    // built at runtime rather than committed as generated vectors.
    const G: &str = "GC6TCMKAV55B5M3ESAJZLEJXSD2KF6UGWXCIFZDB7VURMTLYW724ITS4";
    const C1: &str = "CAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAD2KM";
    const C2: &str = "CAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAFCT4";
    const D3: &str = "CAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAARQG5";
    const D4: &str = "CAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAATYON";

    let env = Env::default();
    assert_protocol_tree(
        &env,
        &[
            (address(&env, G), 100),
            (address(&env, C1), i128::MAX - 106),
            (address(&env, C2), 3),
            (address(&env, D3), 2),
            (address(&env, D4), 1),
        ],
        "0x128928130e416b42c107b220dd1f97a18eb9ab1f815b3bda24ba0f878f639110",
    );
}
...
/// TypeScript passes this ignored test a runtime-generated bundle. Claiming
/// every leaf against the actual contract proves cross-language compatibility
/// without committing generated proof vectors.
#[test]
#[ignore = "invoked by pnpm check:merkle-parity with a TypeScript-generated bundle"]
fn typescript_proofs_are_accepted_by_contract() {
    #[derive(Deserialize)]
    struct Bundle {
        root: StdString,
        claims: StdVec<Claim>,
    ...
    let bundle: Bundle = serde_json::from_str(
        &std::env::var("ZARF_TS_MERKLE_BUNDLE").expect("TypeScript bundle environment variable"),
    )
    .expect("valid TypeScript bundle");

    let env = Env::default();
    env.mock_all_auths();
    let token = env.register(RecordingToken, ());
    let total = bundle.claims.iter().fold(0i128, |sum, claim| {
        sum.checked_add(claim.amount.parse::<i128>().expect("i128 amount"))
            .expect("claim total fits i128")
    });
    let contract = env.register(
        MerkleAirdrop,
        (
            ...
            token.clone(),
            bytes32(&env, &bundle.root),
            total,
            0u64,
            false,
        ),
    );
    let token_client = RecordingTokenClient::new(&env, &token);
    token_client.mint(&contract, &total);
    let client = MerkleAirdropClient::new(&env, &contract);

    for claim in bundle.claims {
        let claimant = address(&env, &claim.address);
        let amount = claim.amount.parse::<i128>().expect("i128 amount");
        let proof = claim
            .proof
            .iter()
            .map(|sibling| bytes32(&env, sibling))
            .collect::<StdVec<_>>();
        ...
        client.claim(
            ...
            &claimant,
            &amount,
            &soroban_proof(&env, &proof),
        );
        assert_eq!(token_client.balance(&claimant), amount);
        assert!(client.is_claimed(&claim.index));
    }
