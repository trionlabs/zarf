...
Rust verifier library for proofs generated from Noir (UltraHonk) on BN254, designed to integrate with Soroban contracts and `soroban-sdk`. This fork targets the bb v2 UltraKeccakHonk proof and verification-key layout used by bb.js v2.1.9.

---

## Features
- Soroban-focused verifier built on `soroban-sdk`  
- Verifies proofs generated from Noir (UltraHonk) using the bb v2 UltraKeccakHonk layout
- Pure Rust core; `no_std` + `alloc` friendly  
- Expects `bb write_vk`
- Legacy bb v0.87 examples remain under `circuits/*/target`, but are ignored by default because this verifier no longer accepts that layout
