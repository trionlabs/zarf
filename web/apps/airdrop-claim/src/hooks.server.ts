// Security headers for the airdrop-claim app.
//
// Mirrors apps/claim's hooks.server.ts but DELIBERATELY OMITS the ZK escape
...
// https://cdn.jsdelivr.net. It also has no Google sign-in, so connect-src omits
// www.googleapis.com. This keeps the airdrop origin on a strict
// default-src 'self' CSP with eval physically unreachable — the security win
// "double mode" buys precisely by NOT co-hosting the airdrop with the ZK claim
// origin (which MUST allow unsafe-eval for its in-browser prover).
//
// HEADERS ONLY: unlike apps/claim this does NOT `import './lib/coreInit'`, so a
...
// IPFS gateways (claim reads the distribution doc by CID) + both Horizons.
...
// Soroban RPC + Horizon + the read indexer. NO pin-proxy (claim never pins).
