...
<section class="flex flex-col items-center justify-center min-h-[60vh] gap-6 text-center px-6">
    <div class="relative flex flex-col items-center justify-center group">
        <!-- Ambient Blur Glow -->
        <div class="absolute w-24 h-24 rounded-full bg-zen-error-muted/30 blur-2xl opacity-60 group-hover:opacity-80 transition-colors duration-500 animate-pulse-glow"></div>
        
        <!-- Yüzen Dış Halka (Floating Outer Ring) -->
        <div class="w-20 h-20 rounded-full border border-zen-border flex items-center justify-center relative transition-all duration-500 group-hover:scale-105 group-hover:border-zen-fg/20 animate-float mb-4">
            ...
            <div class="w-14 h-14 rounded-full flex items-center justify-center shadow-sm bg-zen-bg-elevated/80 backdrop-blur-md border border-zen-border-subtle transition-colors duration-300 group-hover:bg-zen-bg-elevated">
                <AlertTriangle class="w-6 h-6 text-zen-error transition-transform duration-300 group-hover:scale-110" />
            </div>
            
            <!-- Küçük Tasarım Detayı (Accent Dot) -->
            <span class="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-zen-error border border-zen-bg animate-pulse"></span>
        </div>
        <span class="text-xs font-mono uppercase tracking-widest text-zen-fg-muted">Error {status}</span>
    </div>
