<script lang="ts">
    ...
    import { Check, ExternalLink, Loader2, AlertCircle } from 'lucide-svelte';
                ...
                >
                    ...
                    <div
                        class="relative z-10 w-10 h-10 rounded-full flex items-center justify-center
                            ...
                            {status === 'done' ? 'bg-zen-fg text-zen-bg border-zen-fg' : ''}
                            {status === 'active'
...
{#if error}
    <ZenAlert variant="error" class="mb-6 mt-4">
        ...
        {error}
        {#snippet actions()}
            <ZenButton variant="secondary" size="sm" onclick={onRetry}>Retry</ZenButton>
        {/snippet}
    </ZenAlert>
{/if}
