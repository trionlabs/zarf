...
>
    ...
    <div class="relative mb-6 flex items-center justify-center group">
        <!-- Ambient Blur Glow -->
        <div
            class="absolute w-24 h-24 rounded-full {color} blur-2xl opacity-60 group-hover:opacity-80 transition-colors duration-500 animate-pulse-glow"
        ></div>

        <!-- Yüzen Dış Halka (Floating Outer Ring) -->
        <div
            class="w-20 h-20 rounded-full border border-zen-border flex items-center justify-center relative transition-all duration-500 group-hover:scale-105 group-hover:border-zen-fg/20 animate-float"
        >
            ...
            >
                <Icon
                    class="w-6 h-6 {iconColor} transition-transform duration-300 group-hover:scale-110"
                />
            </div>

            <!-- Küçük Tasarım Detayı (Accent Dot) -->
            <span
                class="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-zen-fg/40 border border-zen-bg animate-pulse"
            ></span>
        </div>
    </div>
