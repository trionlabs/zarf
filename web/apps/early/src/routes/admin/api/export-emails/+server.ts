...
export const POST: RequestHandler = async (event) => {
  ...
  // Admin auth: the M1 admin cookie (email-mode primary) OR an allowlisted
  // Subject (secondary). adminId for the audit row is the cookie principal or
  // the subject id.
  const user = event.locals.user;
  const adminCookie = event.cookies.get(ADMIN_COOKIE);
  const cookieOk =
    !!env.ADMIN_KEY && !!adminCookie && (await verifyAdminCookie(env.ADMIN_KEY, adminCookie));
  const subjectOk = isAdmin(user, env.ADMIN_X_USER_IDS, env.ADMIN_EMAILS);
  if (!cookieOk && !subjectOk) {
    return apiError(ERROR_CODES.UNAUTHORIZED, 403, 'admin only');
  }
  const adminId = user?.id ?? 'admin-cookie';
