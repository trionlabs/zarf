...
export const actions: Actions = {
  default: async (event) => {
    ...
    // Forensic trail: the ADMIN_KEY cookie is stateless + replayable-until-
    // expiry, so this success row is the only record of WHEN break-glass was
    // used — the event you most want in an incident. adminId = ip; PII-free.
    // Non-fatal: a failed audit write must not block a valid login.
    try {
      await recordAuditEvent(env.DB, {
        adminId: ip,
