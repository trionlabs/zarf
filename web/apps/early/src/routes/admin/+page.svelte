<script lang="ts">
    ...
    // Funnel stat tiles (Completed is rendered separately with a primary accent).
    const funnelTiles: Array<[string, number]> = $derived([
        ...
        ['Post-verified', data.stats.postVerified],
    ]);
...
<section class="container mx-auto max-w-7xl px-6 py-12 font-sans lg:py-16">
    ...
    {#if showX}
        <div class="mt-10 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-5">
            ...
            {/each}
            <ZenCard variant="glass" padding="md" radius="xl" class="ring-1 ring-inset ring-primary/20">
                <span class="block text-[11px] font-medium uppercase tracking-[0.15em] text-base-content/50">Completed</span>
                <span class="mt-2 block text-3xl font-bold tabular-nums tracking-tight text-primary">{data.stats.completed}</span>
            </ZenCard>
        </div>
        ...
        <div class="mt-6 flex flex-wrap items-center gap-3">
            ...
            <a
                ...
                class="inline-flex items-center justify-center rounded-full bg-zen-btn-secondary-bg px-4 py-2 text-xs font-semibold tracking-[0.02em] text-zen-btn-secondary-text ring-1 ring-inset ring-zen-btn-secondary-border transition-colors hover:bg-zen-btn-secondary-bg-hover hover:text-zen-btn-secondary-text-hover"
            >
                Export CSV
            </a>
        </div>
        ...
        <ZenCard variant="glass" padding="none" radius="2xl" class="mt-6 overflow-hidden">
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    <thead>
                        <tr class="bg-base-content/[0.03] text-left">
                            {#each ['User', 'State', 'Created', 'Attested', 'Verified', 'Post', 'Wallet', 'Email', 'Refs', 'Completed', 'Attempts'] as h (h)}
                    ...
                    <tbody>
                        {#each visible as u (u.id)}
                            ...
                            <tr class="border-t border-base-content/5 transition-colors hover:bg-base-content/[0.03]">
                                ...
                                <td class="whitespace-nowrap px-4 py-3 font-mono text-xs text-base-content/60" title={u.stellar_address ?? ''}>{truncWallet(u.stellar_address)}</td>
                                ...
                                <td class="px-4 py-3">
                                    {#if u.completed_at}
                                        <span class="font-bold text-zen-success" title={fmtDate(u.completed_at)}>✓</span>
                                    {:else}
                                        <span class="text-base-content/40">—</span>
                                    {/if}
                                </td>
