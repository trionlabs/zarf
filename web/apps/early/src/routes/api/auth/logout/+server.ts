...
import { clearSessionCookie, ADMIN_COOKIE } from '$lib/server/session';
...
export const POST: RequestHandler = async (event) => {
  ...
  // Also clear the break-glass admin cookie (stateless, no server-side row) so
  // "Sign out" ends a break-glass session too, not just an X session. Mirror the
  // set attributes so the __Host- delete is honored.
  event.cookies.delete(ADMIN_COOKIE, { path: '/', httpOnly: true, secure: true, sameSite: 'lax' });
