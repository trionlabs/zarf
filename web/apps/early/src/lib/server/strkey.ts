...
/**
 * Decode a canonical Stellar ed25519 account id (G...) to its raw 32-byte
 * ed25519 public key, or null on ANY failure (wrong shape, bad base32, wrong
 * version byte, checksum mismatch). Runs the exact same decode + validate path
 * as isValidStellarAccount, then returns the payload — decoded bytes 1..33
 * (byte 0 is the 0x30 version, bytes 33..35 are the CRC16 checksum).
 *
 * Pure, dependency-free, and NEVER logs the input (a mispasted secret seed must
 * not leak). Used by the wallet-signature verifier to feed the raw pubkey into
 * ed25519 verify.
 */
export function decodeEd25519PublicKey(g: string): Uint8Array | null {
  // Same cheap shape gate as isValidStellarAccount — rejects M.../S..., wrong
  // length and non-base32 chars before touching the decoder or any log line.
  if (!/^G[A-Z2-7]{55}$/.test(g)) return null;

  const decoded = base32Decode(g);
  // 56 base32 chars = 35 bytes: 1 version + 32 key + 2 crc.
  ...
  // Trailing checksum is little-endian over bytes[0..33).
  const expected = decoded[33] | (decoded[34] << 8);
  if (crc16xmodem(decoded, 33) !== expected) return null;

  // Payload = the 32-byte ed25519 public key (drop version byte + CRC).
