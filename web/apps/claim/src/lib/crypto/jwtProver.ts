...
import type { ZKProof, ZKClaimData, GooglePublicKey, MerkleProof } from '@zarf/ui/types';
...
let acvmLib: any = null;
let abiLib: any = null;
let noirLib: any = null;
let aztecLib: any = null;
let noirJwtLib: any = null;
...
async function initializeWasm(): Promise<void> {
    ...
    if (!browser) {
        throw new Error('WASM initialization is not supported on the server side.');
    }

    try {
        // Dynamically load libraries
        [acvmLib, abiLib] = await Promise.all([
            import('@noir-lang/acvm_js'),
            import('@noir-lang/noirc_abi')
        ]);

        const initACVM = acvmLib.default;
        const initAbi = abiLib.default;
...
async function initializeBackend(circuit: any): Promise<{
    ...
    try {
        if (browser) {
            ...
            if (!noirLib) noirLib = await import('@noir-lang/noir_js');
            if (!aztecLib) aztecLib = await import('@aztec/bb.js');

            const { Noir } = noirLib;
            const { UltraHonkBackend } = aztecLib;
            ...
            throw new Error('Proof generation is not supported on the server side.');
        }
...
export async function generateJwtProof(
    ...
    try {
        ...
        // Dynamically load noir-jwt if not loaded
        if (!noirJwtLib) noirJwtLib = await import('noir-jwt');
        const { generateInputs } = noirJwtLib;
