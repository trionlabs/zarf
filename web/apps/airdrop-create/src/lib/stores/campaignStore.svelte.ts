...
function setTokenDetails(details: Partial<TokenDetails>) {
    ...
    invalidatePreparedDeploy();
    persist();
}

// Invalidate a prepared deploy WAL whenever an input that feeds the merkle
// root / total / metadata CID changes after Prepare. Without this, a back-edit
// (add/remove a recipient, change token, deadline, or locked) leaves the WAL's
// STALE root/total/cid bound to a LIVE, mismatched recipient set, deploying a
// permanently-wrong, fund-committed campaign through an ordinary flow
