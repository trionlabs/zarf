...
async function initBarretenberg(): Promise<Barretenberg> {
    if (barretenberg && FrClass) return barretenberg;

    // Prevent concurrent initialization
    if (initPromise) return initPromise;

    if (!browser) {
        throw new Error('Barretenberg hashing is not supported on the server side.');
    }

    initPromise = (async () => {
        try {
            const aztecLib = await import('@aztec/bb.js');
            FrClass = aztecLib.Fr;

            barretenberg = await aztecLib.Barretenberg.new();
            return barretenberg;
        } catch (error) {
...
export async function pedersenHashBytes(bytes: Uint8Array): Promise<bigint> {
    ...
    const fields = Array.from(bytes).map((byte) => new FrClass(BigInt(byte)));
...
export async function pedersenHashPair(left: bigint, right: bigint): Promise<bigint> {
    ...
    const hash = await bb.pedersenHash([new FrClass(left), new FrClass(right)], 0);
...
export async function pedersenHashField(field: bigint): Promise<bigint> {
    const bb = await initBarretenberg();
    const hash = await bb.pedersenHash([new FrClass(field)], 0);
    return BigInt(hash.toString());
}
...
export async function computeIdentityCommitment(email: string, code: string | bigint): Promise<bigint> {
    ...
    if (typeof code === 'bigint') {
        ...
        const codeField = new FrClass(code);
    ...
    const identity = await bb.pedersenHash([new FrClass(emailHash), new FrClass(codeHash)], 0);
    ...
    for (let i = 0; i < entries.length; i++) {
        ...
        for (let epoch = 0; epoch < epochs; epoch++) {
            ...
            if (epoch > 0) {
                ...
                const result = await bb.pedersenHash([new FrClass(currentSecret)], 0);
