...
1. **ONLY DaisyUI Components** - No custom UI wrappers (Button.svelte, Input.svelte, etc.)
2. **ONLY 2 Approved Themes** - `nord` (default) and `wireframe` - NO other themes allowed
3. **NO Custom Styling** - No custom colors, shadows, or glassmorphism in app.css
...
1. **ONLY DaisyUI Components:** Never create custom UI component wrappers (Button.svelte, Input.svelte, etc.)
2. **ONLY Built-in Themes:** Use ONLY DaisyUI's built-in themes - NO custom theme definitions
3. **Approved Themes (2):** ONLY `nord` and `wireframe` - NO other themes allowed
...
| ❌ **FORBIDDEN** | Using any DaisyUI theme other than `nord` or `wireframe` |
| ❌ **FORBIDDEN** | Defining custom themes in app.css or tailwind config |
| ✅ **REQUIRED** | Use native DaisyUI classes: `btn`, `btn-primary`, `input`, `card`, `modal`, `badge` |
| ✅ **REQUIRED** | Use DaisyUI built-in themes via `@plugin "daisyui" { themes: nord, wireframe }` |
...
### 3.3 Theme Selection (RESTRICTED)

* **⚠️ ONLY 2 Approved Themes:** `nord` (default), `wireframe` (alternative)
* **🚫 DO NOT use:** light, dark, cupcake, bumblebee, emerald, corporate, synthwave, retro, cyberpunk, valentine, halloween, garden, forest, aqua, lofi, pastel, fantasy, luxury, dracula, cmyk, autumn, business, acid, lemonade, night, coffee, winter, dim, sunset (ALL FORBIDDEN)
...
2. **Theme Compliance:** Am I using ONLY `nord` or `wireframe` themes? (No other themes, no custom theme definitions)
3. **Logic Check:** Did I remove all `&&` logic from the HTML? (Moved to `$derived`).
4. **Type Check:** Are all props typed? (No `any`).
5. **Runes:** Is reactivity explicit and optimized? (Using `$state.raw` for big objects?).
6. **Theme Compatibility:** Does the component look correct in BOTH `nord` AND `wireframe` themes?
7. **ZK Safety:** If this triggers a proof, is the main thread unblocked?
8. **Security:** Are there any console logs of private inputs?
